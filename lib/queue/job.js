...
/**
 * Allocate resources for beeing used by the job. If the resources do not
 ...
 * @api public
 */
Job.prototype.useResources = function(resource, delay, callback) {
  // XXX transform to single key
  this.resources = resources;
  
  var exe = this.client.multi().hmget("q:limits", resources);
  for (i in resources) {
    exe = exe.hincrby("q:resources", resources[i], 1);    
  }
  
  var self = this;
  // allocate resources
  exe.exec(function(err, results) {
    // check resources
    if(err) {
        this.log("error executing resource allocation:" + err)
    ...
    for(var i = 0; i < results[0].length; i++) {
      limits[resources[i]] = results[0][i];
    }
    
    // update resources list
    var over_limit = false;
    for(var i = 1; i < results.length; i++) {
      var key = resources[i-1];
      var num_resources = parseInt(results[i]);
      over_limit = num_resources > limits[key];
      if(over_limit) break;
    }
    if(over_limit && delay > 0) {
      self.log("resource limitation hit: " + over_limit);
...
Job.prototype.freeResources = function(callback){
  ...
  for (i in this.resources) {
    var key = this.resources[i];
    exe = exe.hincrby("q:resources", key, -1);
    released.push(key);
  }
  var self = this;
  ...
  exe.exec(function(err, res) {
    ...
    if(exe.queue.length > 1) {
      exe.exec(function(err, res) {callback(allocated);});
    } else {
      callback(allocated);
    }
  });
