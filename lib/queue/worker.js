 ...
 * Is called when a deadly signal is received, but may be useful when
 * jobserver goes down.
 *
 * @api public
 */
...
Worker.prototype.emergencyRelease = function(in_exit) {
  ...
  if(todo == 0 && !in_exit)
    ...
    console.log("kue: emergency resource release of " + todo + " jobs");
...
Worker.prototype.process = function(job, fn){
  ...
  job.active();
    ...
    job.freeResources(function(allocated) {
      if (!allocated) {
          // XXX hack until can support cross process job events
          new Job("spider", {
